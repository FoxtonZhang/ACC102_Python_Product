...
if run_button:
    if not wrds_username or not wrds_password:
        ...
        try:
            ...
            # 创建 4x2 的图表布局，增加画布高度 / Create a 4x2 chart layout, increase figure height
            fig, axes = plt.subplots(4, 2, figsize=(20, 24))

            # 图1：累计收益率 / Chart 1: Cumulative Return
            sns.lineplot(data=df_final, x='date', y='cum_return', hue='ticker', ax=axes[0, 0])
            axes[0, 0].set_title('Cumulative Return (Growth of $1)', fontsize=14, fontweight='bold')
            ...
            sns.barplot(data=metrics_df.sort_values('Annual_Volatility'), x='ticker', y='Annual_Volatility',
                        ax=axes[1, 0])
            axes[1, 0].set_title('Annualized Volatility (Total Risk)', fontsize=14, fontweight='bold')
            ...
            sns.scatterplot(data=metrics_df, x='Annual_Volatility', y='Annual_Return', hue='ticker', s=200,
                            ax=axes[1, 1])
            axes[1, 1].set_title('Risk vs Return Trade-off', fontsize=14, fontweight='bold')
            ...
            sns.kdeplot(data=df_final, x='daily_return', hue='ticker', fill=True, ax=axes[2, 0])
            axes[2, 0].set_title('Distribution of Daily Returns', fontsize=14, fontweight='bold')
            axes[2, 0].set_xlim(-0.1, 0.1)

            # 图6：滚动年化波动率 / Chart 6: Rolling Volatility (126-day)
            sns.lineplot(data=df_final, x='date', y='rolling_vol', hue='ticker', ax=axes[2, 1])
            axes[2, 1].set_title('Rolling 126-day Volatility', fontsize=14, fontweight='bold')

            # 图7：资产相关性热力图 / Chart 7 (New): Asset Correlation Heatmap
            sns.heatmap(corr_matrix, annot=True, cmap='coolwarm', vmin=0, vmax=1, ax=axes[3, 0], square=False,
                        fmt=".2f")
            axes[3, 0].set_title('Asset Correlation Matrix', fontsize=14, fontweight='bold')

            # 图8：收益率箱线图与尾部风险 / Chart 8 (New): Boxplot & Tail Risk
            sns.boxplot(data=df_final, x='ticker', y='daily_return', ax=axes[3, 1], palette="Set2")
            axes[3, 1].set_title('Return Distribution & Tail Risk (Boxplot)', fontsize=14,
                                 fontweight='bold')
            ...
            st.markdown(
                "Copy the prompt below to ChatGPT/Claude for expert analysis.")
